...
The repository includes a Python script (i.e., wfip3_analysis.py) that performs the analysis and requires standard scientific Python libraries (e.g., NumPy, pandas, matplotlib, etc.). The script computes probability-density distributions for several flow variables and compares them with standard WT metrics. The analysis script is located in scripts/ and changes into the data/ directory using os.chdir("../data") to load the data files. This means that the WFIP3 data files must be placed in a data/ folder one directory above the script; the results images are manually stored in the results/ directory. The figures will be saved in the same directory as the data files. Finally, WFIP3 provides one file for each day of observations, so the full period consists of multiple daily files.
